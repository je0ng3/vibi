...
vibi `:cmp` — **Compose Multiplatform UI 모듈**. 타임라인 에디터, 음원 분리/조절 UI, 미리보기, 채팅 패널, 로그인 화면 등 Android/iOS 공통 화면을 단일 `@Composable` 코드로 작성. 비즈니스 로직(도메인·리포지토리)은 형제 모듈 `:shared` 에서 가져온다.
...
└── src/
    ...
    │   │   ├── InsertSubtitleSheet.kt              # 자막 수동 추가 (로컬 표시 전용 — 렌더 burn-in 제거됨)
    ...
    │   ├── ui/chat/ChatPanel.kt                    # Gemini function calling 대화 + 제안 confirm
